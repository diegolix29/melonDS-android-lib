...
bool GLRenderer2D::InitShaders()
{
    ...
    if (!OpenGL::CompileVertexFragmentProgram(LayerPreShader,
                                              shader_2DLayerPreVS, shader_2DLayerPreFS,
                                              "2DLayerPreShader",
                                              {{"vPosition", 0}},
                                              {{"oColor", 0}}))
        return false;
    ...
    if (!OpenGL::CompileVertexFragmentProgram(SpritePreShader,
                                              shader_2DSpritePreVS, shader_2DSpritePreFS,
                                              "2DSpritePreShader",
    ...
    if (!OpenGL::CompileVertexFragmentProgram(SpriteShader,
                                              shader_2DSpriteVS, shader_2DSpriteFS,
                                              "2DSpriteShader",
    ...
    if (!OpenGL::CompileVertexFragmentProgram(CompositorShader,
                                              shader_2DCompositorVS, shader_2DCompositorFS,
                                              "2DCompositorShader",
                                              {{"vPosition", 0}},
                                              {{"oColor", 0}}))
        return false;
...
bool GLRenderer2D::Init()
{
    ...
    for (int j = 0; j < 8; j++)
    {
        ...
        for (int k = 0; k < sz[2]; k++)
        {
            ...
            // glDrawBuffer not available in OpenGL ES - framebuffer is already set
