...
void ScreenshotRenderer::renderScreenshot(GPU* gpu, Renderer renderer, Frame* renderFrame)
{
    if (renderer == Renderer::Software)
    {
        ...
        if (gpu->GetFramebuffers(&topFramebuffer, &bottomFramebuffer))
        {
            memcpy(screenshotBuffer, topFramebuffer, 256 * 192 * 4);
            memcpy(&screenshotBuffer[256 * 192], bottomFramebuffer, 256 * 192 * 4);
        }
    }
    else
    {
