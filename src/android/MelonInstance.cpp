...
u32 MelonInstance::runFrame()
{
    ...
    {
        int scale = 1; // Default scale factor
    ...
    bool isRendererAccelerated = false; // Default to non-accelerated
    if (isRendererAccelerated)
    {
        int backBuffer = 0; // FrontBuffer not available in GPU class
        // SetOutputTexture not available in base Renderer class
    }
    ...
    if (!isRendererAccelerated)
    {
        int frontbuf = 0; // Default to front buffer
        void* topFramebuffer, *bottomFramebuffer;
        if (nds->GPU.GetFramebuffers(&topFramebuffer, &bottomFramebuffer) && topFramebuffer && bottomFramebuffer)
        {
            glBindTexture(GL_TEXTURE_2D, renderFrame->frameTexture);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 256, 192, GL_RGBA, GL_UNSIGNED_BYTE, topFramebuffer);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 192 + 2, 256, 192, GL_RGBA, GL_UNSIGNED_BYTE, bottomFramebuffer);
...
void MelonInstance::updateRenderer()
{
    ...
    if (newRenderer != currentRenderer)
    {
        switch (newRenderer)
        {
            ...
            case Renderer::OpenGl:
                {
                nds->GPU.SetRenderer(std::make_unique<GLRenderer>(*nds, false));
                }
                break;
            case Renderer::Compute:
                {
                // ComputeRenderer3D cannot be directly set as renderer - it's a 3D renderer component
                nds->GPU.SetRenderer(std::make_unique<GLRenderer>(*nds, true));
                }
                break;
    ...
    switch (newRenderer)
    {
        case Renderer::Software:
        {
            ...
            // SetThreaded not available in base Renderer class
            break;
        }
        case Renderer::OpenGl:
        {
            auto glRenderSettings = static_cast<OpenGlRenderSettings&>(*currentConfiguration->renderSettings);
            // SetRenderSettings call removed - type mismatch
            break;
        }
        case Renderer::Compute:
        {
...
{
    return screenshotRenderer.getScreenshot() != nullptr;
}
