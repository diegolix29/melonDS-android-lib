...
bool GLRenderer3D::BuildRenderShader(bool wbuffer)
{
    ...
    std::string vsbuf = shader_3DRenderVS;
    if (wbuffer)
    {
        auto pos = vsbuf.find('\n') + 1;
    ...
    std::string fsbuf = shader_3DRenderFS;
    if (wbuffer)
    {
        auto pos = fsbuf.find('\n') + 1;
...
bool GLRenderer3D::Init()
{
    ...
    // glDepthRange not available in OpenGL ES
    glClearDepthf(1.0);

    if (!OpenGL::CompileVertexFragmentProgram(ClearShaderPlain,
            shader_3DClearVS, shader_3DClearFS,
            "ClearShaderPlain",
            {{"shader_vPosition", 0}},
            {{"shader_oColor", 0}, {"shader_oAttr", 1}}))
        return false;
    ...
    if (!OpenGL::CompileVertexFragmentProgram(ClearShaderBitmap,
              shader_3DClearBitmapVS, shader_3DClearBitmapFS,
              "ClearShaderBitmap",
              {{"vPosition", 0}},
              {{"oColor", 0}, {"oAttr", 1}}))
        return false;
    ...
    if (!OpenGL::CompileVertexFragmentProgram(FinalPassEdgeShader,
            shader_3DFinalPassVS, shader_3DFinalPassEdgeFS,
            "FinalPassEdgeShader",
            {{"vPosition", 0}},
            {{"oColor", 0}}))
        return false;
    if (!OpenGL::CompileVertexFragmentProgram(FinalPassFogShader,
            shader_3DFinalPassVS, shader_3DFinalPassFogFS,
            "FinalPassFogShader",
            {{"vPosition", 0}},
            {{"oColor", 0}}))
        return false;
...
void GLRenderer3D::RenderFrame()
{
    ...
    glBindBuffer(GL_UNIFORM_BUFFER, ShaderConfigUBO);
    // glMapBuffer not available in OpenGL ES - use glBufferData instead
    glBufferData(GL_UNIFORM_BUFFER, sizeof(ShaderConfig), &ShaderConfig, GL_STATIC_DRAW);
    // glUnmapBuffer not available in OpenGL ES
