...
#else
    // Android doesn't support compute shaders, always use GLRenderer3D
    IsCompute = false;
    Rend3D = std::make_unique<GLRenderer3D>(GPU.GPU3D, *this);
#endif
...
bool GLRenderer::Init()
{
    ...
    if (!OpenGL::CompileVertexFragmentProgram(FPShader,
                                              shader_FinalPassVS, shader_FinalPassFS,
                                              "2DFinalPassShader",
                                              {{"vPosition", 0}},
    ...
    if (!OpenGL::CompileVertexFragmentProgram(CaptureShader,
                                              shader_CaptureVS, shader_CaptureFS,
                                              "2DCaptureShader",
    ...
    if (!OpenGL::CompileVertexFragmentProgram(CapDownShader,
                                              shader_CaptureDownscaleVS, shader_CaptureDownscaleFS,
...
#undef SETVERTEX
    ...
    // glDrawBuffer not available in OpenGL ES
